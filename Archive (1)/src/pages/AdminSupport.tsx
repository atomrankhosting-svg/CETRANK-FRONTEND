...
export default function AdminSupport() {
  ...
  const fetchTickets = async () => {
    try {
      ...
      if (stored) {
        ...
        // Seed with a mock ticket for demo purposes if empty
  ...
  // Set default values when a ticket is selected
  useEffect(() => {
    if (selectedTicket) {
  ...
  const handleUpdateTicket = async (e: React.FormEvent) => {
    ...
    try {
      if (isFallback) {
        ...
        toast({
          title: "Ticket Updated (Local)",
          description: "Changes saved to simulated database.",
        });
      } else {
  ...
  const handleDeleteTicket = async (id: string) => {
    ...
    try {
      if (isFallback) {
        ...
        toast({
          title: "Ticket Deleted",
          description: "Support ticket deleted from Supabase.",
        });
      }
    } catch (error: any) {
      ...
      toast({
        ...
        description: "Could not remove the ticket.",
        variant: "destructive",
      });
    }
  };
      ...
      <main className="relative z-10 mx-auto max-w-6xl px-4 pb-12 pt-32">
          ...
          <div className="mb-6 flex gap-3 rounded-2xl border border-amber-500/20 bg-amber-500/5 p-4 text-amber-600 text-sm">
            ...
            <div>
              ...
              <p className="text-xs mt-1 text-amber-600/80">
                The database table `support_tickets` was not detected in Supabase. We are loading and saving tickets inside your browser's local storage so you can fully test this page. Run the SQL query in `supabase_schema.sql` to connect your live Supabase database.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="grid gap-4 grid-cols-2 md:grid-cols-4 mb-8">
          ...
          <div className="glass rounded-2xl border border-border/50 p-4">
            ...
            <h4 className="text-2xl font-bold mt-1 text-red-500">{openCount}</h4>
          </div>
          <div className="glass rounded-2xl border border-border/50 p-4">
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">In Progress</p>
            <h4 className="text-2xl font-bold mt-1 text-amber-500">{progressCount}</h4>
          </div>
          <div className="glass rounded-2xl border border-border/50 p-4">
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Resolved</p>
            <h4 className="text-2xl font-bold mt-1 text-emerald-500">{resolvedCount}</h4>
          </div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-12">
          ...
          {/* Right panel: Ticket details and responses */}
